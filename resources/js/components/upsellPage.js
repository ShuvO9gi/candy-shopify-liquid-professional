...
const show_package = () => {
  ...
  let upsellHeaderHeight = document.querySelector(
    ".drawer_header_shopAdjust"
  ).clientHeight;
  let upsellFooterHeight = document.querySelector(
    ".drawer_footer_shopAdjust---upsell"
  ).clientHeight;
  let upsellTotalHeight = document.querySelector(
    ".drawerContainer_shopAdjust"
  ).clientHeight;
  document.querySelector(
    ".drawer_content_shopAdjust--upsell"
  ).style.height = `${
    upsellTotalHeight - (upsellHeaderHeight + upsellFooterHeight)
  }px`;
...
const show_cross_sell = () => {
  ...
  document
    .querySelector("[data-drawer-upsell]")
    .querySelector(".btn-text").innerHTML = "gå til betaling";
...
export default function beforeCheckout(cart_data, next = false) {
  let has_pkg =
    ...
    document.querySelectorAll(".sa-upsell-item").length
      ...
      : false;
  let has_cross_sell =
    document.querySelectorAll(".sa-upsell-detail___item") &&
    document.querySelectorAll(".sa-upsell-detail___item").length
      ? true
      : false;
