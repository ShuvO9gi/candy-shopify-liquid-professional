...
const imgUrlDefault = '//cdn.shopify.com/s/files/1/0555/3461/6756/files/slikeks-logo-2022.png';
...
export default component((node, ctx) => {
  ...
  const {
    handle, totalProducts, paginate, priceLimit, itemWidth, minusIcon, plusIcon
  ...
  ctx.on("products:reload", state => {
    ...
    itemInners.forEach((wrapper, i) => {
        ...
        wrapper.querySelector('[data-qty-wrapper]').classList.remove('is--visible');
...
const decodeHtml = (html)  => {
  ...
  const addToCardEventListeners = () => {
    ...
    const productCardBubbles = node.querySelectorAll('[data-product-card-bubble]');
    ...
    const infoModalDescription = node.querySelector('[data-info-modal-description]');
        ...
        qtyIdArray.diff(idArray).forEach((id) => {
          qtyWrappers.forEach((wrapper, i) => {
            if (Number(wrapper.dataset.qtyId) === id) {
              ...
              const { count, amount, price, weight } = fromLocalStorage.find((storageItem) => storageItem.id === id);
              // qtyCountElems[i].innerHTML = ` ${price / 100} `;
              qtyCountElems[i].innerHTML = ` ${weight} `;
              itemInners[i]?.classList.add('is--selected');
              amountElems[i] != undefined ? amountElems[i].innerHTML = amount * count : '';
              amountContainers[i] != undefined ? amountContainers[i].classList.remove('hidden') : '';
              productCardBubbles[i] != undefined ? productCardBubbles[i].classList.remove('opacity-0') : '';
            }
          });
        });
      }

      visualCandyBag(visualBagWrapper, visualBagElem, visualBagToggleBtns, candyBag, priceLimit, fromLocalStorage);
    }

    const addToBag = (elem, i) => {
      const {
        id, price, weight, amount, title, image,
      } = elem.dataset;
      const candy = {};
      ...
      const identicalCandy = candyBag.find((candyItem) => candyItem.title === title);
    ...
    plusBtns.forEach((plusBtn, i) => {
      plusBtn.addEventListener('click', (e) => {
        ...
        plusCandy(addToBagBtns, candyBag, qtyCountElems, amountContainers, amountElems, i);
        setToLocalStorage(fromLocalStorage, candyBag);
        visualCandyBag(visualBagWrapper, visualBagElem, visualBagToggleBtns, candyBag, priceLimit, fromLocalStorage);
        totalKilogramsCalculator(candyBag, itemCount, totalCount, priceLimit, submitBtns);
        productCardBubbles[i].classList.remove('opacity-0');
    ...
    minusBtns.forEach((minusBtn, i) => {
      minusBtn.addEventListener('click', (e) => {
        e.preventDefault();
        minusCandy(addToBagBtns, qtyWrappers, candyBag, qtyCountElems, amountContainers, amountElems, itemInners, submitBtns, i);
        setToLocalStorage(fromLocalStorage, candyBag);
        visualCandyBag(visualBagWrapper, visualBagElem, visualBagToggleBtns, candyBag, priceLimit, fromLocalStorage);
        if (candyBag.length > 0) {
          totalKilogramsCalculator(candyBag, itemCount, totalCount, priceLimit, submitBtns);
    ...
    submitBtns.forEach(submitBtn => {
      submitBtn.addEventListener('click', (e) => {
        ...
        if (bag) {
          ...
          document.querySelector('[data-name-prompt-skip]').addEventListener('click', () => {
            namePromptModal.classList.remove('is--visible');
          });
        }
        ...
        finalSubmit.addEventListener('click', (e) => {
          ...
          if (found) {
            ...
            node.querySelector('[data-ask-rename-headline]').innerHTML =  decodeHtml(strings.title);
            node.querySelector('[data-ask-rename-description]').innerHTML =  decodeHtml(strings.description);
            node.querySelector('[data-ask-rename-submit]').innerHTML =  decodeHtml(strings.accept);
            node.querySelector('[data-ask-rename-decline]').innerHTML = decodeHtml(strings.decline);
            ...
            setTimeout(() => {
              const renamePromptModal = node.querySelector('[data-ask-rename-prompt]');
              
              if (node.querySelectorAll('[data-ask-rename-title]')) {
                node.querySelectorAll('[data-ask-rename-title]').forEach(item => {
                  item.innerHTML = `"${name}"`
                })	
              }
              ...
              node.querySelector('[data-ask-rename-prompt-close]').addEventListener('click', () => {
                renamePromptModal.classList.remove('is--visible');
              });
    
              node.querySelector('[data-ask-rename-submit]').addEventListener('click', () => {
                renamePromptModal.classList.remove('is--visible');
                // add to cart  with same name 
                const items = createItemsArray(candyBag, name, found.properties['ID'],note,note2,note3);
                if (items.length > 0) {
                  // Submit the list to the cart
                  submitBagToCart(items, successPopup, namePromptModal);
                  namePromptModal.classList.remove('is--visible');
              ...
              node.querySelector('[data-ask-rename-decline]').addEventListener('click', () => {
                renamePromptModal.classList.remove('is--visible');
                namePromptModal.classList.remove('is--visible');
                // show prompt name
                namePromptModal.classList.add('is--visible');
          ...
          if (name === '') {
            ...
            const items = createItemsArray(candyBag, name, identifier,note,note2,note3);
            if (items.length > 0) {
              // Submit the list to the cart
              submitBagToCart(items, successPopup, namePromptModal);
              namePromptModal.classList.remove('is--visible');
  ...
  const createCollectionItem = (product) => {
    const {
      title, images, variants, price, description, vendor, tags,
    } = product; 

    const imgUrl = images[0];
    const { id } = variants[0];
    const amountTag = tags?.find((tag) => tag.includes('amount__')); // Find tag to control the amount each candy that's put in the bag
    const amount = amountTag === undefined ? 4 : Number(amountTag.replace('amount__', '')); // If no tag is found, set the default amount to 4
    const weightTag = tags?.find((tag) => tag.includes('weight__')); // Find tag to control the weight each candy that's put in the bag
    const weight = weightTag === undefined ? 20 : Number(weightTag.replace('weight__', '')); // If no tag is found, set the default amount to 20
        ...
        <div class="" data-item-inner>
          ...
          <div class="candyBoxBuySection">
            <div class="candyBoxBuySectionContent">
              ...
              <div class="candyBoxBrand type-style-card-h-subhead">${theme.strings.blandSelv.from} <span>${vendor.toLowerCase()}</span></div>
              <button class="candyBoxBuyButton type-style-button-text" data-add-to-bag data-id="${id}" data-amount="${amount}" data-price="${price}" data-weight="${weight}" data-title="${title}" data-image="${imgUrl}"> ${theme.strings.blandSelv.addToBag} </button>
            </div>
          </div>
            ...
            <div class="candyBoxBuySectionContent">
              ...
              <div class="candyBoxBuyButtonClicked">
                ...
                <div class="candySizes" tracking-wide>
                  <div class="candySizesAmount type-style-card-desc-bold" data-amount-container>${theme.strings.blandSelv.ca_stk} <span data-amout-stick data-amount-elem>${amount}</span> ${theme.strings.blandSelv.stks}</div>
                  <div class="candySizesAmountBorder"></div>
                  <div class="candySizesWeigth type-style-card-desc-regular"><span class="mx-[3px]" data-qty-amount data-init-weight="${weight}" data-init-price="${price}">${weight}</span> ${theme.strings.blandSelv.gram}</div>
                </div>
  ...
  const renderProducts = (products) => {
    ...
    let results = productsToRender.length > 0
