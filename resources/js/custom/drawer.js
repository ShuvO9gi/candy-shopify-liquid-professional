...
export default component((node, ctx) => {
    ...
    const triggers = [...document.querySelectorAll(`[href="/cart"]`),
    ...
    const grand_total_el = node.querySelector('[data-original-total-grand-total]');
    ...
    ctx.on('drawer:show', state => {
        document.querySelector('[data-drawer-cart--upsell]').setAttribute('opened', false);
        document.querySelector('.drawer-cart-inner').setAttribute('hide', false);

        setTimeout(() => {
            let hh = document.querySelector(".drawer_footer_shopAdjust").clientHeight + 120;
            // console.log(hh)
            document.querySelector(".drawer_content_shopAdjust").style.minHeight = `calc(100% - ${hh}px)`
        }, 500)
    ...
    ctx.on('drawer:upsell', state => {
        ...
        document.querySelector(".sa-cross_sell-wrapper").style.minHeight = `calc(100% - ${hh}px)`
    ...
    ctx.on("drawer:updateItems", state => {
        ...
        if (bag) {
            ...
            let total = cart_data.items.map(list => {
                if (list.properties?.hasOwnProperty('ID') && list.properties.ID == bag.properties.ID) {
            ...
            engine.parseAndRender('{{total | money : cart.currency }}', {
                cart: cart_data,
                total: total,
                bag_name: bag_name
            }).then(html => {
    ...
    const purchaseLimitHandler = (e) => {
        ...
        if (purchaseLimit <= cartTotal) {
            ...
            node.querySelector('[ data-minimum-purchase-alert]').classList.add('is--hidden')
           
        }
        else {
            dataSubmitCart.setAttribute("disabled", true)
            node.querySelector('[ data-minimum-purchase-alert]').classList.remove('is--hidden')
        }

        let countDown = purchaseLimit - cartTotal
        
        if (countDown > 0) {
            engine.parseAndRender("<strong>{{countdown | divided_by: 100 | money : cart.currency | remove: ',00'}}</strong>", {
                cart: cart_data,
                countdown: countDown
            }).then(html => {
    ...
    const restoreBag = (bagId) => {
        ...
        cart_data.items.forEach(list => {
            if (list.properties?.hasOwnProperty('ID') && list.properties.ID == bagId) {
                const item = {
                    bag_id: bagId,
        ...
        if (plusBtns != null) {
            plusBtns.forEach((btn, idx) => {
                ...
                btn.addEventListener('click', (e) => {
                    e.preventDefault();
                    const currentQuantitys = document.querySelectorAll('[data-cartItem-quantity]');
                    let currentQuantity = Number(currentQuantitys[idx].dataset.cartitemQuantity);
    ...
    const render = async () => {
        ...
        cart.items.forEach(item => {
            if (item.properties?.hasOwnProperty('ID')) {
                ...
                cart.items.forEach(list => {
                    if (list.properties?.hasOwnProperty('ID') && list.properties.ID == item.properties.ID) {
                        subItems.push(list)
        ...
        const listenCart = () => {
            return engine.parseAndRender(CartTemplateSelector.innerHTML, { cart: cart })
        ...
        listenCart()
            .then(() => {
                ...
                if (cart.original_total_price > shippingPriceLimit || cart.item_count <= 0) shipping_amount = 0;

                const total = cart.original_total_price + shipping_amount;

                engine.parseAndRender('{{cart.items_subtotal_price | divided_by: 100 | money : cart.currency }}', {
                    cart: cart_data,
            ...
            .then(() => {
                ...
                if (ads <= 10) ads = 50;
                let hh = document.querySelector(".drawer_footer_shopAdjust").clientHeight + 70 + ads;
                document.querySelector(".drawer_content_shopAdjust").style.minHeight = `calc(100% - ${hh}px)`
                ctx.emit('drawer:updated');
            }).catch(() => {
                ctx.emit('drawer:updated');
    ...
    const open = () => {
        render().then(() => {
            ...
            if (cart_data?.item_count != oldCart?.item_count || cart_data?.total_price != oldCart?.total_price || cart_data?.total_weight != oldCart?.total_weight) {
    ...
    const clean = () => {
        return location.pathname.includes('collections/bland-selv-slik') != false ? ctx.emit('products:fetched', ctx.getState()) : false;
    }
    ...
    const callback_item = () => {
        ...
        lineItems.forEach((item, i) => {
            ...
            if (stk <= 0 && allProducts) {
                current_product = allProducts.find(product => product.handle == cart_item.handle);
            }
            
            if (current_product) {
                const amountTag = current_product.tags?.find((tag) => tag.includes('amount__'));
                stk = amountTag === undefined ? 4 : Number(amountTag.replace('amount__', ''));
            }
            ...
            itemQtyMinusBtn.addEventListener('click', debounce(function () {
            ...
            itemQtyPlusBtn.addEventListener('click', debounce(function () {
                drawer_loader(true, node.querySelector('#sl-cartjs-items'));
                updateDrawerLineItem(obj).then(data => {
                    render_cart(cart_item);
                })
            ...
            deleteElems[i].addEventListener('click', debounce(function () {
                obj[itemId] = 0;
                item.parentNode.removeChild(item)
                drawer_loader(true, node.querySelector('#sl-cartjs-items'));
                updateDrawerLineItem(obj).then(data => {
    ...
    const renderItems = async (id, node) => {
        ...
        const CartTemplateSelector = document.querySelector('#sl-CartTemplate-items');
        const cartAjaxSelector = document.querySelector('#sub-products');
        cartAjaxSelector.classList.add('progress')
        return engine.parseAndRender(CartTemplateSelector.innerHTML, { cart: cart_data, items: items, subtotal: 10, bag_id: id })
            .then(html => cartAjaxSelector.innerHTML = html)
            .then(() => {
    ...
    const hideItems = (id, node) => {
        ...
        if (node)
            node.classList.remove('second-opened');
        else document.querySelector('#drawer-cart').classList.remove('second-opened');
    }
    ...
    document.addEventListener('click', e => {
        ...
        if (!container.contains(e.target) && !triggers.includes(e.target) && !e.target.classList.contains('minusOne_shopAdjust')) {
            close();
        }
    })
